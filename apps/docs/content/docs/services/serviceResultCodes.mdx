...
|---|---|---|---|
| Missing required parameter | 400 | application/json | `{"status": 400, "title": "Bad Request", "detail": "Invalid request.", "type": "https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/400", "errors": ["countryCode is required"]}` |
| Communication failure with external service | 502 | application/json | `{"status": 502, "title": "Bad Gateway", "detail": "Server got an invalid response.", "type": "https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/502"}` |
| Timeout | 504 | application/json | `{"message": "Endpoint request timed out"}` |